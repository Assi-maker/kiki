...
5. `evaluate_heuristics`/`decide_pre_entry`/`decide_open_position` never read `guardian_authority_shadow_heuristics` (grep-provable: the only call sites of `find_guardian_authority_heuristics()` are inside `decide_pre_entry`'s and `decide_open_position`'s own callers in `authority.py`/`tick.py`, and neither ever calls a `_shadow_` variant). **Correction (post-implementation, final review, 2026-09-15): `guardian_authority_shadow_heuristics` will in practice stay permanently empty, and this is expected, not a bug.** The shadow engine reads the SAME real `guardian_authority_heuristics` table the production engine reads (by design — it's evaluating what the real engine *would* have decided). With that table empty (the very cold-start deadlock this plan exists to work around), `evaluate_heuristics` always sums to `0.0`, so every shadow decision is `NO_ACTION` forever, and Task 8's self-critique (TIGHTEN_SL-only, per its own established scope) can never have a row to learn from. This plan closes the *data-accumulation* deadlock (a rich corpus of resolved `NO_ACTION` shadow rows, each carrying real factors and the real closed position's real outcome, ready for human review) — it deliberately does NOT close the *automated-learning* deadlock, since doing so without a human-authored or otherwise-sourced first real heuristic would require exactly the fabricated "seed" heuristic the user explicitly refused. `guardian_authority_shadow_heuristics`/Task 8's self-critique pass remain live, tested, and ready — they simply have nothing to act on until a real heuristic exists in the real table, at which point they activate automatically with zero further code changes.
