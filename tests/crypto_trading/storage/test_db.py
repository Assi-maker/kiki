...
def test_guardian_authority_decisions_table_has_matched_heuristic_ids_json_column_on_a_fresh_database(
    tmp_path,
):
    conn = get_connection(tmp_path / "test.db")
    columns = {
...
def test_migration_adds_matched_heuristic_ids_json_column_to_a_pre_existing_database_without_it(
    tmp_path,
):
    """Task 2 (2026-09-15, Guardian Authority Live Autonomy):
    guardian_authority_decisions.matched_heuristic_ids_json was added AFTER
    Guardian Authority's own table-creation code (including the
    ...
    pattern and same verification style as the intervention_applied
    migration above: build the table via the OLD shape (no
    matched_heuristic_ids_json column), reopen via get_connection() (a real
    restart), and prove the migration adds the column without destroying the
    existing row."""
    db_path = tmp_path / "pre_existing.db"
    ...
    old_conn.execute(
        ...
        "outcome_status TEXT NOT NULL DEFAULT 'PENDING', actual_exit_reason TEXT, "
        "actual_pnl_usdt TEXT, expectation_correct BOOLEAN, resolved_at TEXT, old_sl TEXT, "
        "new_sl TEXT, run_id TEXT NOT NULL, intervention_applied BOOLEAN)"
    )
    old_conn.execute(
        "INSERT INTO guardian_authority_decisions (decision_id, position_id, candidate_id, "
        "decision_type, decided_at, reasoning, expected_outcome, expected_direction, "
        "confidence, run_id) VALUES "
        "('ga-old-2', 'pos-1', 'cand-1', 'TIGHTEN_SL', '2026-08-01T00:00:00+00:00', "
        "'reasoning', 'expect favorable', 'favorable', 0.7, 'run-1')"
    )
    old_conn.commit()
    old_conn.close()
    ...
    row = conn.execute(
        "SELECT * FROM guardian_authority_decisions WHERE decision_id = 'ga-old-2'"
    ).fetchone()
    assert row is not None
    assert row["decision_type"] == "TIGHTEN_SL"
...
def test_migration_matched_heuristic_ids_json_is_idempotent_across_repeated_connections(tmp_path):
    db_path = tmp_path / "test.db"
    get_connection(db_path)
    conn2 = get_connection(db_path)  # second connection must not crash on ALTER TABLE again
    columns = {
