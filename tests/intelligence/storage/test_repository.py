...
def test_save_assessment_attaches_to_opportunity(repo):
    ...
    repo.save_opportunity(opp)
    qa = QAAssessment(
        agent_name="qa-agent", run_id="r1", created_at=datetime.now(UTC),
        status="ok", passed=True, violations=[],
    )
...
def test_log_run_event_does_not_raise(repo):
    repo.log_run_event(
        run_id="r1", event_id="evt-1", opportunity_id=None,
        agent_name="orchestrator", status="started",
    )
